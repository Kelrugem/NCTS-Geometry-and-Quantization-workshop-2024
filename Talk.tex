...
\vspace{.5pt}
LGB $\pi_{\mathcal{G}} \colon {\mathcal{G}} \to M$
\begin{center}
	\begin{tikzcd}[ampersand replacement=\&]
{\mathcal{G}} \arrow[d, shift left, "\pi_G"]\arrow["\pi_G", swap, shift right]{d} \\
	M
	\end{tikzcd}
\end{center}
\end{example}
...
\begin{center}
	\begin{tikzcd}[ampersand replacement=\&]
x \& \arrow[l, swap, "{(x, q)}" pos=.42, bend right] x \cdot q \& \arrow[l, swap, "{(x \cdot q, q')}" pos= .45, bend right] \arrow[ll, swap, "{(x, q) ~ (x \cdot q, q')}", bend right=70] x \cdot q q'
	\end{tikzcd}
\end{center}
...
\vspace{.5pt}
Lie group $G$ with action $\Psi \colon N \times G \to N$, $(x, q) \mapsto x \cdot q$, on $N$. 
\begin{center}
	\begin{tikzcd}[ampersand replacement=\&]
...
p \cdot e_{\Phi(p)} &= p
\eas
for all $(p, g, g') \in P \fibtimes{\Phi}t \mathcal{G} \fibtimes st \mathcal{G}$.
\end{definition}
\end{frame}
...
\begin{frame}{Outlook}
\begin{itemize}
	\item More applications? So far: Singular foliations \& Symmetry Breaking
	\item Local examples?
\end{itemize}
\end{frame}
